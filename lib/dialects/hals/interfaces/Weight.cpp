...
std::shared_ptr<std::vector<T>> WeightOp::read() {
  ...
  auto hal_tensortype = cast<HalTensorType>(getOutput().getType());
  auto type = hal_tensortype.GetRankedTensorType();
...
template <typename T>
mlir::Value WeightOp::create(mlir::Operation *OwnerOp, llvm::StringRef suffix,
  ...
  HalTensorType hal_type =
      HalTensorType::get(ctx,type);
  auto newOp =
      builder.create<WeightOp>(NameLoc::get(nameAttr), hal_type, ValueRange{});
...
template <typename T>
mlir::LogicalResult WeightOp::update(const std::vector<T> &data, size_t count) {
  auto op = getOperation();
...
mlir::Value WeightOp::clone_bf16(mlir::Operation *OwnerOp, std::string name) {
  auto type = cast<HalTensorType>(getType()).GetRankedTensorType();
...
#pragma omp parallel for schedule(static, omp_schedule(count))
  ...
  auto nameAttr = builder.getStringAttr(new_name);
  auto newOp =
      builder.create<WeightOp>(NameLoc::get(nameAttr), HalTensorType::get(ctx,new_type,TensorKind::WEIGHT,MemorySpace::L1), ValueRange{});
  return newOp.getResult();
}
mlir::Value WeightOp::clone_f16(mlir::Operation *OwnerOp) {
  auto type = cast<HalTensorType>(getType()).GetRankedTensorType();
  auto dtype = type.getElementType();
  assert(dtype.isF32());
  auto data = read<float>();
...
#pragma omp parallel for schedule(static, omp_schedule(count))
  ...
  auto nameAttr = builder.getStringAttr(new_name);
  auto newOp =
      builder.create<WeightOp>(NameLoc::get(nameAttr), new_type, ValueRange{});
  return newOp.getResult();
}
mlir::Value WeightOp::clone_int(mlir::Operation *OwnerOp) {
...
#pragma omp parallel for schedule(static, omp_schedule(count))
  ...
  assert(succeeded(ret));
  auto nameAttr = builder.getStringAttr(new_name);
  auto newOp = builder.create<WeightOp>(NameLoc::get(nameAttr), HalTensorType::get(ctx,new_type,TensorKind::WEIGHT,MemorySpace::L1),
                                             ValueRange{});
  return newOp.getResult();
}
mlir::Value WeightOp::clone_f8e4m3(mlir::Operation *OwnerOp,
...
#pragma omp parallel for schedule(static, omp_schedule(count))
  ...
  auto new_type = RankedTensorType::get(
      type.getShape(),
      builder.getFloat8E4M3FNType()); // builder.getFloat8E5M2Type());
  auto ret =
...
mlir::Value WeightOp::clone_f8e5m2(mlir::Operation *OwnerOp) {
    auto type = cast<HalTensorType>(getType()).GetRankedTensorType();
  auto dtype = type.getElementType();
  assert(dtype.isF32());
  auto data = read<float>();
...
mlir::Value WeightOp::clone(llvm::StringRef suffix) {
  auto ctx = getContext();
  OpBuilder builder(ctx);
  auto op = getOperation();
  ...
  assert(succeeded(ret));
  auto newOp =
      builder.create<WeightOp>(NameLoc::get(nameAttr), getType(), ValueRange{});
  return newOp.getOutput();
}
mlir::Value WeightOp::split(int begin, int end, int axis, mlir::Type to_type,
                            std::string suffix) {
  auto op = getOperation();
