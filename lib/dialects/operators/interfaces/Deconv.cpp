...
deconv_attr_t ops::DeconvOp::parseParam() {
  deconv_attr_t p = {0};
  auto ishape = cast<RankedTensorType>(getInput().getType()).getShape();
  auto oshape = cast<RankedTensorType>(getOutput().getType()).getShape();
  ...
  p.with_bias = !isa<NoneType>(getBias().getType());
  p.g = getGroup();
  p.n = ishape[0];
  p.ic = ishape[1];
  p.oc = oshape[1];
...
void ops::DeconvOp::shape_inference() {
