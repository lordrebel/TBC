...
struct MergeToRMSNormPattern : public OpRewritePattern<ReciprocalOp> {
  ...
  LogicalResult matchAndRewrite(ReciprocalOp op,
    ...
    if (op.getConstVal().convertToDouble() != 1.0) {
      return failure();
    }
    ...
    if (!pow_op || pow_op.getNumOperands() != 2 ||
        pow_op.getOperand(0) != pow_op.getOperand(1) ) {
      return failure();
    }
    ...
    if (weight_mul_op) {
      ...
      if (weight_mul_op.getNumOperands() != 2 ||
          !weight) {
        return failure();
      }
      ...
      auto scale = cast<WeightOp>(scale_op.getScale().getDefiningOp());
      auto bias = cast<WeightOp>(scale_op.getBias().getDefiningOp());
