...
static bool parseParam(StridedSliceOp op,
  ...
  auto start_op =
      dyn_cast_or_null<WeightOp>(op.getStarts().getDefiningOp());
  auto end_op =
      dyn_cast_or_null<WeightOp>(op.getEnds().getDefiningOp());
  auto stride_op =
      dyn_cast_or_null<WeightOp>(op.getStrides().getDefiningOp());
...
static void rewriteParam(stridedslice_param_t_t param, StridedSliceOp op) {
  ...
  auto new_starts = WeightOp::create(op, "mergerd", *param.start_v, starts_ranked_type);
  ...
  auto new_ends = WeightOp::create(op, "mergerd", *param.end_v, ends_ranked_type);
  ...
  auto new_strides = WeightOp::create(op, "mergerd", *param.stride_v, strides_ranked_type);
