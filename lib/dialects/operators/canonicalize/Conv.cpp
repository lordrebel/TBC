...
struct Conv3dTranspose : public OpRewritePattern<ConvOp> {
  ...
  LogicalResult matchAndRewrite(ConvOp op,
    ...
    auto filter_op = filter.getDefiningOp<ops::WeightOp>();
...
struct Conv1x1Convkxk2dMerge : public OpRewritePattern<ConvOp> {
  ...
  LogicalResult matchAndRewrite(ConvOp op,
    ...
    if (!prevConvOp) {
      ...
      auto preFilterop = prevConvOp.getFilter().getDefiningOp<ops::WeightOp>();
...
#pragma omp parallel for schedule(static, omp_schedule(eckk))
      ...
      if (p.has_bias) {
        auto Biasop = op.getBias().getDefiningOp<ops::WeightOp>();
      ...
      if (prep.has_bias) {
        ...
        auto preBiasop = prevConvOp.getBias().getDefiningOp<ops::WeightOp>();
...
#pragma omp parallel for schedule(static, omp_schedule(E))
      ...
      auto mnew_op = ops::WeightOp::create(op, "merge_filter_weight",
      ...
      if (p.has_bias || prep.has_bias) {
        ...
        auto mbnew_op = ops::WeightOp::create(op, "merge_bias_weight",
