...
struct Permute5dSplit : public OpRewritePattern<PermuteOp> {
  ...
  LogicalResult matchAndRewrite(PermuteOp op,
    ...
    auto permute0_op =
        rewriter.create<PermuteOp>(loc, outType, operands, attrs);
    ...
    auto outType1 = cast<RankedTensorType>(op.getResult().getType());
    auto permute1_op =
        rewriter.create<PermuteOp>(op.getLoc(), outType1, operands, attrs);
...
// [4,3,28,1] => [4,3,1,28]
// [4,3,1,28] => [4,1,3,28]
struct PermuteToReshape : public OpRewritePattern<PermuteOp> {
  using OpRewritePattern::OpRewritePattern;
  LogicalResult matchAndRewrite(PermuteOp op,
                                PatternRewriter &rewriter) const override {
    // todo
    ...
    if (do_reshape && order->size() == 2 && order->at(0) == 1 &&
        order->at(1) == 0) {
      auto nonzeroOp = dyn_cast<NonZeroOp>(op.getInput().getDefiningOp());
...
//      -Reshape-Permute-MatMul-Permute-Reshape-Unsqueeze-
// clang-format on
struct DecomposedRelPosEmb : public OpRewritePattern<PermuteOp> {
  using OpRewritePattern::OpRewritePattern;
...
void PermuteOp::getCanonicalizationPatterns(RewritePatternSet &results,
  ...
  results.insert<TopPermuteToPixelShuffle, TopPermuteToReorg, Permute5dSplit,
                 PermuteFuse, PermuteToReshape, NonZeroPermutePattern,
                 DecomposedRelPosEmb>(context);
}
