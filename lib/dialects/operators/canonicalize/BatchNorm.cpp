...
struct BatchNormToScale : public OpRewritePattern<BatchNormOp> {
  using OpRewritePattern::OpRewritePattern;
  BatchNormToScale(MLIRContext *context, PatternBenefit benefit = 9)
...
void BatchNormOp::getCanonicalizationPatterns(RewritePatternSet &results,
                                              MLIRContext *context) {
  results.insert<BatchNormToScale>(context);
}
