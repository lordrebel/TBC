...
// reshape (in == out)
struct FuseReshape2 : public OpRewritePattern<ReshapeOp> {
...
struct FuseReshape3 : public OpRewritePattern<ReshapeOp> {
  using OpRewritePattern::OpRewritePattern;
...
struct InValidReshapeMergePattern : public OpRewritePattern<ReshapeOp> {
  ...
  LogicalResult matchAndRewrite(ReshapeOp op,
    ...
    for (auto nextOp : op.getResult().getUsers()) {
      ...
      if (auto unsqueezeOp = dyn_cast<UnsqueezeOp>(nextOp)) {
...
void ReshapeOp::getCanonicalizationPatterns(RewritePatternSet &results,
                                            MLIRContext *context) {
  results.insert<patterns::FuseRepeatPattern<ReshapeOp>, FuseReshape2,
                 FuseReshape3, ReshapeInstanceNormPattern, MergeGeluPattern,
                InValidReshapeMergePattern>(context);
}
