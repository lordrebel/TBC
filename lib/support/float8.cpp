...
static uint8_t fp32_to_fp8(const fp32 single, bool is_e5m2, bool saturate) {
  ...
  if (single.format.exp > (127 - FP8_EXP_BIAS) && single.format.exp < 0xff) {
    ...
    uint32_t tmp = Right_Shift_Round(single.bits, shift_num);
    ...
    mantissa = Right_Shift_Round(mantissa, shift_num);
    res = mantissa & 0x7f;
...
uint8_t f32_to_f8e4m3(float src) {
  fp32 tmp = {.fval = src};
  return fp32_to_fp8(tmp, false, true);
}

uint8_t f32_to_f8e5m2(float src) {
  fp32 tmp = {.fval = src};
  return fp32_to_fp8(tmp, true, true);
}
