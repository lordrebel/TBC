...
#include <float.h>
//using ROUNDING_HALF_AWAY_FROM_ZERO as round
namespace tbc {
...
static inline bf16 fp32_to_bf16_denorm(fp32 src) {
  bf16 dst;
...
static inline bf16 fp32_to_bf16_all(fp32 &src) {
  bf16 dst;
  fp32 fp32val;
  long long temp_r, temp_l;
  if (src.format.exp > 0 && src.format.exp < 255) {

    temp_r = RightShiftRound<long long>((long long)src.bits, 16);
...
static inline fp16 fp32_to_fp16_all(fp32 &src) {
  fp16 dst;
  fp32 fp32val;
  long long temp_r, temp_l;
  ...
  if (src.format.exp > 112 && src.format.exp < 255) {
      ...
      temp_r = RightShiftRound<long long>(src.bits, 13);
    ...
    mant = RightShiftRound<long long>(mant, rshift_num);
    mant = src.format.sign ? (0 - mant) : mant;
