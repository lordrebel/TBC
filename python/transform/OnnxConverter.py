...
onnx_attribute_translator = {
    ...
    "dtype": lambda x: onnx_data_type(x),
    "keepdims": lambda x: bool(x),
    "to": lambda x: onnx_data_type(x),
}

# Function to convert ONNX data type to Python data type
def onnx_data_type(dtype):
...
def translate_onnx(key, val):
    return onnx_attribute_translator.get(key, lambda x: x)(val)
...
class OnnxConverter(BaseConverter):
    ...
    def __init__(self,
        ...
        self.dtype_map={
            ...
            onnx.TensorProto.FLOAT8E4M3FN: 'FLOAT8E4M3',
        ...
        self.onnxop_factory = {
            ...
            "TopK": lambda node: self.convert_operatorsk_op(node),
    ...
    def get_value_infos(self, model: onnx.ModelProto):
        for v in model.graph.value_info:
            shape = self.get_shape_from_value_info_proto(v)
            dtype = self.dtype_map[v.type.tensor_type.elem_type]
            self.addValueInfo(v.name, shape, dtype)
        for i in model.graph.input:
            if i.name not in self.value_infos:
                shape = self.get_shape_from_value_info_proto(i)
                dtype = self.dtype_map[i.type.tensor_type.elem_type]
                self.addValueInfo(i.name, shape, dtype)
        for o in model.graph.output:
            if o.name not in self.value_infos:
                shape = self.get_shape_from_value_info_proto(o)
                dtype = self.dtype_map[o.type.tensor_type.elem_type]
                self.addValueInfo(o.name, shape, dtype)
    ...
    def get_input_types(self, model: onnx.ModelProto):
        ...
        for input in self.get_inputs(model):
          ...
          input_types.append(self.value_infos[input.name].dtype)
    ...
    def get_output_types(self, model: onnx.ModelProto):
        ...
        for output in self.get_outputs(model):
          output_types.append(self.value_infos[output.name].dtype)
        return output_types
    ...
    def get_input_shapes(self, model: onnx.ModelProto):
        ...
        return [self.value_infos[i.name].shape for i in inputs]
    ...
    def load_onnx_model(self, onnx_file, input_shapes: list, output_names: list, without_simplfy=True):
        ...
        for tensor in self.model.graph.initializer:
            ...
            if(tensor.data_type in [onnx.TensorProto.FLOAT8E4M3FN,onnx.TensorProto.FLOAT8E5M2, onnx.TensorProto.FLOAT8E4M3FNUZ]):
                data=data.view(np.uint8)
            if(tensor.data_type in [onnx.TensorProto.FLOAT16,onnx.TensorProto.BFLOAT16]):
                data=data.view(np.uint16)
            self.addWeight(name, data,self.dtype_map[tensor.data_type])
    ...
    def input_shape_assign(self, input_shapes):
        ...
        for idx, input in enumerate(inputs):
            ...
            self.value_infos[input.name].shape = _shape
    ...
    def convert_skip_op(self, onnx_node):
        ...
        if onnx_node.outputs[0] in self.value_infos:
            if(onnx_node.inputs[0]) not in self.value_infos or self.value_infos[onnx_node.inputs[0]].shape != self.value_infos[onnx_node.outputs[0]].shape:
                self.value_infos[onnx_node.inputs[0]] = self.value_infos[onnx_node.outputs[0]]
                self.updateOperandTensorType(onnx_node.inputs[0])
    ...
    def convert_add_op(self, onnx_node):
        ...
        new_op = operators.AddOp(self.valueInfoToTensorType(onnx_node.outputs[0]), [lhs_op, rhs_op],
    ...
    def convert_sub_op(self, onnx_node):
        ...
        if self.isScalar(lhs):
            # lhs_const + (-1 * rhs)
            rhs_op = self.getOp(rhs)
            new_op = operators.SubConstOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                                    rhs_op,
            ...
            # lhs + (-rhs_const)
            lhs_op = self.getOp(lhs)
            new_op = operators.AddConstOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                                    lhs_op,
            ...
            lhs_op = self.getOp(lhs)
            rhs_op = self.getOp(rhs)
            new_op = operators.SubOp(self.valueInfoToTensorType(onnx_node.outputs[0]), [lhs_op, rhs_op],
                               loc=self.get_loc(name),
                               ip=self.mlir.insert_point).output
        self.addOperand(onnx_node.name, new_op)
    ...
    def convert_batchnorm_op(self, onnx_node):
        ...
        new_op = operators.BatchNormOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                                 op,
                                 mean,
                                 variance,
                                 gamma,
    ...
    def convert_ceil_op(self, onnx_node):
        ...
        op = self.getOp(onnx_node.inputs[0])
        new_op = operators.CeilOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                            op,
    ...
    def convert_concat_op(self, onnx_node):
        ...
        for x in onnx_node.inputs:
            if self.isWeight(x):
                ...
                if weight_data is not None:
                    w_name = x + "_weight"
                    self.addWeight(w_name, weight_data,weight_dtype)
                    operands.append(self.getWeightOp(w_name))
                    weight_data = None
                operands.append(self.getOp(x))
        if len(operands) == 0:
            # all weight
            self.addWeight(onnx_node.name, weight_data,weight_dtype)
            return
        if weight_data is not None:
        ...
        if len(operands) == 1:
            ...
            return
        new_op = operators.ConcatOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                              operands,
                              axis=axis,
                              loc=self.get_loc("{}_{}".format(onnx_node.name, onnx_node.op_type)),
                              ip=self.mlir.insert_point).output
    ...
    def convert_constant_op(self, onnx_node):
        ...
        tensor = onnx_node.attrs['value']
        data = numpy_helper.to_array(tensor)
        if(tensor.data_type in [onnx.TensorProto.FLOAT8E4M3FN,onnx.TensorProto.FLOAT8E5M2, onnx.TensorProto.FLOAT8E4M3FNUZ]):
                data=data.view(np.uint8)
        if(tensor.data_type in [onnx.TensorProto.FLOAT16,onnx.TensorProto.BFLOAT16]):
                data=data.view(np.uint16)
        self.addWeight(onnx_node.name, data,self.dtype_map[tensor.data_type])
    ...
    def convert_constantofshape_op(self, onnx_node):
        ...
        if 'value' in onnx_node.attrs:
            ...
            value = np_tensor[0]
        op = self.getOp(onnx_node.inputs[0])
        new_op = operators.ConstantFillOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                                    op,
                                    value=value,
                                    loc=self.get_loc("{}_{}".format(onnx_node.name,
                                                                    onnx_node.op_type)),
    ...
    def convert_conv_op(self, onnx_node):
        ...
        new_op = operators.ConvOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                            *operands,
    ...
    def convert_depth2space_op(self, onnx_node):
        ...
        new_op = operators.Depth2SpaceOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                                   op,
                                   block_h=blocksize,
                                   block_w=blocksize,
                                   is_CRD=(mode != b"DCR"),
    ...
    def convert_flatten_op(self, onnx_node):
        ...
        new_op = operators.FlattenOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                               op,
                               start_dim=axis,
                               loc=self.get_loc("{}_{}".format(onnx_node.name, onnx_node.op_type)),
                               ip=self.mlir.insert_point).output
    ...
    def convert_floor_op(self, onnx_node):
        ...
        op = self.getOp(onnx_node.inputs[0])
        new_op = operators.FloorOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                             op,
                             loc=self.get_loc("{}_{}".format(onnx_node.name, onnx_node.op_type)),
                             ip=self.mlir.insert_point).output
        self.addOperand(onnx_node.name, new_op)
        ...
        if self.isWeight(A):
            if trans_a == 1 or alpha != 1:
                ...
                if alpha != 1:
                    _tensor *= alpha
                A += '_fix'
                self.addWeight(A, _tensor,self.getDtype(onnx_node.inputs[0]))
            operands.append(self.getWeightOp(A))
        else:
            operands.append(self.getOp(A))
        ...
        if self.isWeight(B):
            if trans_b == 1 or alpha != 1:
                ...
                if alpha != 1:
                    _tensor *= alpha
                B += '_fix'
                self.addWeight(B, _tensor,self.getDtype(onnx_node.inputs[1]))
            operands.append(self.getWeightOp(B))
        else:
            operands.append(self.getOp(B))
        if len(onnx_node.inputs) > 2 and beta != 0:
            ...
            if self.isWeight(C):
                if beta != 1:
                    ...
                    _tensor *= beta
                    C += '_fix'
                    self.addWeight(C, _tensor,self.getDtype(onnx_node.inputs[2]))
                operands.append(self.getWeightOp(C))
            else:
                operands.append(self.getOp(C))
        else:
            operands.append(self.mlir.none_op)

        new_op = operators.MatMulOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                              *operands,
                              loc=self.get_loc("{}_{}".format(onnx_node.name, onnx_node.op_type)),
                              ip=self.mlir.insert_point).output
        self.addOperand(onnx_node.name, new_op)
    ...
    def convert_global_maxpool_op(self, onnx_node):
        ...
        op = self.getOp(onnx_node.inputs[0])
        new_op = operators.MaxPoolOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                               op,
                               kernel_shape=[],
                               strides=[],
                               pads=[],
    ...
    def convert_global_avgpool_op(self, onnx_node):
        ...
        op = self.getOp(onnx_node.inputs[0])
        # check onnx define
        new_op = operators.AvgPoolOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                               op,
                               kernel_shape=[],
                               strides=[],
                               pads=[],
    ...
    def convert_mul_op(self, onnx_node):
        ...
        op0 = self.getOp(lhs)
        op1 = self.getOp(rhs)
        mul_op = operators.MulOp(self.valueInfoToTensorType(onnx_node.outputs[0]), [op0, op1],
                            loc=self.get_loc(name),
                            ip=self.mlir.insert_point).output
    ...
    def convert_relu_op(self, onnx_node):
        ...
        op = self.getOp(onnx_node.inputs[0])
        new_op = operators.ReluOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                            op,
                            loc=self.get_loc("{}_{}".format(onnx_node.name, onnx_node.op_type)),
                            ip=self.mlir.insert_point).output
        self.addOperand(onnx_node.name, new_op)
    ...
    def convert_leaky_relu_op(self, onnx_node):
        ...
        op = self.getOp(onnx_node.inputs[0])
        new_op = operators.LeakyReluOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                                 op,
    ...
    def convert_reshape_op(self, onnx_node):
        ...
        if self.isWeight(onnx_node.inputs[1]):
            ...
            new_op = operators.ReshapeOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                                op,
                                shape=shape,
                                loc=self.get_loc("{}_{}".format(onnx_node.name, onnx_node.op_type)),
                                ip=self.mlir.insert_point).output
        else:
            shape = self.getOp(onnx_node.inputs[1])
            new_op = operators.ReshapeOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                                   op,
                                   shapeT=shape,
                                   loc=self.get_loc("{}_{}".format(onnx_node.name, onnx_node.op_type)),
    ...
    def resize_to_interp(self, onnx_node, op, scale_h, scale_w, mode,
        ...
        new_op = operators.InterpOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                              op,
                              target_shape,
    ...
    def convert_resize_op(self, onnx_node):
        ...
        if (use_size):
            ...
            self.addWeight(onnx_node.name + "_target_shape",
                           np.array(scale_factor[2:], dtype=np.int64),"INT64")
    ...
    def convert_shape_op(self, onnx_node):
        ...
        new_op = operators.ShapeOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                             op,
                             start=start,
                             end=end,
    ...
    def convert_range_op(self, onnx_node):
        ...
        range_op = operators.RangeOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                             start,
                             limit,
                             delta,
                             loc=self.get_loc("{}_{}".format(onnx_node.name, onnx_node.op_type)),
    ...
    def convert_sigmoid_op(self, onnx_node):
        ...
        new_op = operators.SigmoidOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                               op,
                               scale=scale,
                               bias=bias,
                               loc=self.get_loc("{}_{}".format(onnx_node.name, onnx_node.op_type)),
    ...
    def convert_sign_op(self, onnx_node):
        ...
        op = self.getOp(onnx_node.inputs[0])
        new_op = operators.SignOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                            op,
                            loc=self.get_loc("{}_{}".format(onnx_node.name, onnx_node.op_type)),
                            ip=self.mlir.insert_point).output
        self.addOperand(onnx_node.name, new_op)
    ...
    def convert_sin_op(self, onnx_node):
        ...
        op = self.getOp(onnx_node.inputs[0])
        new_op = operators.SinOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                           op,
                           loc=self.get_loc("{}_{}".format(onnx_node.name, onnx_node.op_type)),
                           ip=self.mlir.insert_point).output
        self.addOperand(onnx_node.name, new_op)
    ...
    def convert_cos_op(self, onnx_node):
        ...
        op = self.getOp(onnx_node.inputs[0])
        new_op = operators.CosOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                           op,
                           loc=self.get_loc("{}_{}".format(onnx_node.name, onnx_node.op_type)),
                           ip=self.mlir.insert_point).output
        self.addOperand(onnx_node.name, new_op)
    ...
    def convert_slice_op(self, onnx_node):
        def try_get_slice_input(node, i, attr):
            ...
            if is_const:
                ...
                ret_op = self.getOp(node.inputs[i])
        ...
        if num_input > 1:
            op = self.getOp(onnx_node.inputs[0]) if not self.isWeight(onnx_node.inputs[0]) \
            ...
            if steps[0] == -1 and starts[0] == -1 and ends == -np.iinfo(np.int64).max and self.isWeight(op):
                ...
                self.addWeight(indices_op_name, np_tensor,"INT64")
                indices_op = self.getWeightOp(indices_op_name)
                extra_attr.update({"keepdims": True})
                indices = indices_op
                new_op = operators.GatherOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                        in0,
                        indices,
                        axis=axes,
                        **extra_attr,
            ...
            if not (starts_is_const * ends_is_const * axes_is_const * steps_is_const):
                new_op = operators.SliceAxisOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                                         op,
                                         axis_op,
                                         start_op,
                                         step_op,
        ...
        if self.isWeight(onnx_node.inputs[0]):
            ...
            self.addWeight(onnx_node.name, tensor_data, self.getDtype(onnx_node.inputs[0]))
            return
        op = self.getOp(onnx_node.inputs[0])
        new_op = operators.SliceOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                             op,
                             self.mlir.none_op,
                             self.mlir.none_op,
                             self.mlir.none_op,
    ...
    def convert_transpose_op(self, onnx_node):
        ...
        new_op = operators.PermuteOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                               op,
    ...
    def convert_softmax_op(self, onnx_node):
        ...
        new_op = operators.SoftmaxOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                               op,
                               axis=axis,
    ...
    def convert_softplus_op(self, onnx_node):
        ...
        op = self.getOp(onnx_node.inputs[0])
        new_op = operators.SoftplusOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                                op,
                                loc=self.get_loc("{}_{}".format(onnx_node.name, onnx_node.op_type)),
                                ip=self.mlir.insert_point).output
        self.addOperand(onnx_node.name, new_op)
    ...
    def convert_log_op(self, onnx_node):
        ...
        op = self.getOp(onnx_node.inputs[0])
        new_op = operators.LogOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                           op,
                           loc=self.get_loc("{}_{}".format(onnx_node.name, onnx_node.op_type)),
                           ip=self.mlir.insert_point).output
        self.addOperand(onnx_node.name, new_op)
    ...
    def convert_exp_op(self, onnx_node):
        ...
        op = self.getOp(onnx_node.inputs[0])
        new_op = operators.ExpOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                           op,
                           loc=self.get_loc("{}_{}".format(onnx_node.name, onnx_node.op_type)),
                           ip=self.mlir.insert_point).output
        self.addOperand(onnx_node.name, new_op)
    ...
    def convert_elu_op(self, onnx_node):
        ...
        op = self.getOp(onnx_node.inputs[0])
        new_op = operators.EluOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                           op,
                           alpha=onnx_node.attrs.get("alpha", 0.),
                           loc=self.get_loc("{}_{}".format(onnx_node.name, onnx_node.op_type)),
                           ip=self.mlir.insert_point).output
    ...
    def convert_erf_op(self, onnx_node):
        ...
        op = self.getOp(onnx_node.inputs[0])
        new_op = operators.ErfOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                           op,
                           loc=self.get_loc("{}_{}".format(onnx_node.name, onnx_node.op_type)),
                           ip=self.mlir.insert_point).output
        self.addOperand(onnx_node.name, new_op)
    ...
    def convert_pad_op(self, onnx_node):
        ...
        new_op = operators.PadOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                           op,
                           paddings=pads,
                           val=val,
                           mode=StringAttr.get(mode),
    ...
    def convert_div_op(self, onnx_node):
        ...
        if self.isScalar(lhs):
            # lhs_const * (1 / rhs)
            rhs_op = self.getOp(rhs)
            new_op = operators.ReciprocalOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                                      rhs_op,
                                      const_val=self.getScalar(lhs),
                                      loc=self.get_loc(name),
                                      ip=self.mlir.insert_point).output
            ...
            # lhs * (1 / rhs_const)
            lhs_op = self.getOp(lhs)
            new_op = operators.MulConstOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                                    lhs_op,
            ...
            lhs_op = self.getOp(lhs)
            rhs_op = self.getOp(rhs)
            new_op = operators.DivOp(self.valueInfoToTensorType(onnx_node.outputs[0]), [lhs_op, rhs_op],
                               loc=self.get_loc(name),
                               ip=self.mlir.insert_point).output
        self.addOperand(onnx_node.name, new_op)
    ...
    def convert_reciprocal_op(self, onnx_node):
        ...
        op0 = self.getOp(onnx_node.inputs[0])
        div_op = operators.ReciprocalOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                                  op0,
                                  const_val=1,
                                  loc=self.get_loc("{}_{}".format(onnx_node.name,
                                                                  onnx_node.op_type)),
    ...
    def convert_squeeze_op(self, onnx_node):
        ...
        new_op = operators.SqueezeOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                               op,
                               loc=self.get_loc("{}_{}".format(onnx_node.name, onnx_node.op_type)),
    ...
    def convert_unsqueeze_op(self, onnx_node):
        ...
        if self.opset < 13:
            ...
            if len(onnx_node.inputs) == 1:
                ...
                axes = self.getWeight(onnx_node.inputs[1]).astype(int)
        new_op = operators.UnsqueezeOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                                 op,
                                 loc=self.get_loc("{}_{}".format(onnx_node.name,
                                                                 onnx_node.op_type)),
                                 ip=self.mlir.insert_point,
    ...
    def convert_clip_op(self, onnx_node):
        ...
        if min == 0.0 and max > min:
            ...
            new_op = operators.ClipOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                                input,
                                min=min,
                                max=max,
                                loc=self.get_loc("{}_{}".format(onnx_node.name, onnx_node.op_type)),
    ...
    def convert_conv_transpose_op(self, onnx_node):
        ...
        if weight_name in self.tensors:
            ...
            if weight_name not in self.mlir.load_weight:
                ...
                self.Setshape(weight_name,self.tensors[weight_name].shape)
        ...
        new_op = operators.DeconvOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                              *operands,
                              kernel_shape=kernel_shape,
                              strides=strides,
                              dilations=dilations,
    ...
    def convert_split_op(self, onnx_node):
        ...
        new_op = operators.SplitOp([self.valueInfoToTensorType(i) for i in onnx_node.outputs],
                             op,
                             axis=axis,
                             num=num_output,
    ...
    def convert_reduce_op(self, onnx_node):
        ...
        axes.sort()
        new_op = operators.ReduceOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                              op,
                              axes=axes,
                              keepdims=keepdims,
    ...
    def convert_lrn_op(self, onnx_node):
        ...
        new_op = operators.LRNOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                           op,
                           size=size,
                           alpha=alpha,
                           beta=beta,
    ...
    def convert_gru_op(self, onnx_node):
        ...
        operands = list()
        operands.append(self.getOp(onnx_node.inputs[0]))  # in
        ...
        out_op = operators.GRUOp(*self.mlir.get_tensor_type(out_shapes),
                           *operands,
    ...
    def convert_lstm_op(self, onnx_node):
        ...
        for idx, out in enumerate(onnx_node.outputs):
            if len(out) > 0 and self.check_need(out):
                ...
                out_needs[idx] = True
                out_shapes[idx] = []
        out_op = operators.LSTMOp(*self.mlir.get_tensor_type(out_shapes),
                            *operands,
                            hidden_size=hidden_size,
                            bidirectional=direction == b'bidirectional',
                            batch_first=batch_first,
    ...
    def convert_gather_elements_op(self, onnx_node):
        ...
        name = "{}_{}".format(onnx_node.name, onnx_node.op_type)
        indices = self.getOp(onnx_node.inputs[1])
        new_op = operators.GatherElementsOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                                      in0,
                                      indices,
                                      axis=axis,
                                      loc=self.get_loc(name),
    ...
    def convert_gathernd_op(self, onnx_node):
        ...
        name = "{}_{}".format(onnx_node.name, onnx_node.op_type)
        new_op = operators.GatherNDOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                                input,
                                indices,
    ...
    def convert_expand_op(self, onnx_node):
        ...
        if self.isWeight(onnx_node.inputs[1]):
            shape = self.getWeight(onnx_node.inputs[1])
            new_op = operators.ExpandOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                                   in0,
                                   shape=shape,
                                   loc=self.get_loc("{}_{}".format(onnx_node.name, onnx_node.op_type)),
                                   ip=self.mlir.insert_point).output
    ...
    def convert_tile_op(self, onnx_node):
        ...
        if self.isWeight(onnx_node.inputs[1]):
            ...
            if np.prod(tile_data) == 1:
                ...
                new_op = operators.TileOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                        in0_op,
                        tile = tile_data,
                        loc=self.get_loc("{}_{}".format(onnx_node.name, onnx_node.op_type)),
                        ip=self.mlir.insert_point).output
        else:
            tile_op = self.getOp(onnx_node.inputs[1])
            new_op = operators.TileOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                        in0_op,
                        tileT = tile_op,
                        loc=self.get_loc("{}_{}".format(onnx_node.name, onnx_node.op_type)),
    ...
    def convert_operatorsk_op(self, onnx_node):
        assert (onnx_node.op_type == "TopK")
        in_op = self.getOp(onnx_node.inputs[0])
        ...
        if (len(onnx_node.inputs) > 1):
            if self.isWeight(onnx_node.inputs[1]):
                ...
                k_op = self.getOp(onnx_node.inputs[1])
        axis = onnx_node.attrs.get('axis', -1)
        ...
        out_needs = [False, False]
        for idx, out in enumerate(onnx_node.outputs):
            #operatorsk at the hw need two output
            if len(out) > 0:
                loc_names[idx] = "{}_{}".format(out, onnx_node.op_type)
                out_needs[idx] = True
                out_shapes[idx] = []
        out_op = operators.TopKOp(*self.mlir.get_tensor_type(out_shapes),
                            in_op,
                            axis=axis,
                            K=K,
                            kT=k_op,
    ...
    def convert_max_op(self, onnx_node):
        ...
        new_op = operators.MaxOp(self.valueInfoToTensorType(onnx_node.outputs[0]), [lhs_op, rhs_op],
                           loc=self.get_loc("{}_{}".format(onnx_node.name, onnx_node.op_type)),
                           ip=self.mlir.insert_point).output
        self.addOperand(onnx_node.name, new_op)
    ...
    def convert_min_op(self, onnx_node):
        ...
        lhs_op = self.getWeightOp(lhs) if self.isWeight(lhs) else self.getOp(lhs)
        rhs_op = self.getWeightOp(rhs) if self.isWeight(rhs) else self.getOp(rhs)
        new_op = operators.MinOp(self.valueInfoToTensorType(onnx_node.outputs[0]), [lhs_op, rhs_op],
                           loc=self.get_loc("{}_{}".format(onnx_node.name, onnx_node.op_type)),
                           ip=self.mlir.insert_point).output
        self.addOperand(onnx_node.name, new_op)
    ...
    def convert_abs_op(self, onnx_node):
        ...
        new_op = operators.AbsOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                           operand,
                           loc=self.get_loc("{}_{}".format(onnx_node.name, onnx_node.op_type)),
                           ip=self.mlir.insert_point).output
        self.addOperand(onnx_node.name, new_op)
    ...
    def convert_neg_op(self, onnx_node):
        ...
        name = "{}_{}".format(onnx_node.name, onnx_node.op_type)
        operand = self.getOp(onnx_node.inputs[0])
        mul_const_op = operators.MulConstOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                                      operand,
                                      const_val=-1.0,
                                      loc=self.get_loc(name),
                                      ip=self.mlir.insert_point).output
    ...
    def convert_nms_op(self, onnx_node):
        ...
        nms_op = operators.NmsOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                           operands,
                           center_point_box=0,
    ...
    def convert_prelu_op(self, onnx_node):
        ...
        prelu_op = operators.PReluOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                               in_op,
                               slope,
                               loc=self.get_loc("{}_{}".format(onnx_node.name, onnx_node.op_type)),
                               ip=self.mlir.insert_point).output
    ...
    def convert_sum_op(self, onnx_node):
        ...
        opd0 = self.getOp(onnx_node.inputs[0])
        num_inputs = len(onnx_node.inputs)
        for i in range(1, num_inputs):
            opd1 = self.getOp(onnx_node.inputs[i])
            last_name = onnx_node.name
            if i != num_inputs - 1:
                last_name += "_{}".format(str(i))
            opd0 = operators.AddOp(self.valueInfoToTensorType(onnx_node.outputs[0]), [opd0, opd1],
    ...
    def convert_sqrt_op(self, onnx_node):
        ...
        operand = self.getOp(onnx_node.inputs[0])
        new_op = operators.SqrtOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                            operand,
                            loc=self.get_loc("{}_{}".format(onnx_node.name, onnx_node.op_type)),
                            ip=self.mlir.insert_point).output
        self.addOperand(onnx_node.name, new_op)
    ...
    def convert_tanh_op(self, onnx_node):
        ...
        op = self.getOp(onnx_node.inputs[0])
        new_op = operators.TanhOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                            op,
                            loc=self.get_loc("{}_{}".format(onnx_node.name, onnx_node.op_type)),
                            ip=self.mlir.insert_point).output
        self.addOperand(onnx_node.name, new_op)
    ...
    def convert_pow_op(self, onnx_node):
        ...
        if self.isScalar(expn):
            ...
            if expn_const == 1.0:
                ...
                return
            if expn_const == 2.0:
                mul_op = operators.MulOp(self.valueInfoToTensorType(onnx_node.outputs[0]), [base_op, base_op],
                                   loc=self.get_loc("{}_{}".format(onnx_node.name,
                                                                   onnx_node.op_type)),
                                   ip=self.mlir.insert_point).output
                self.addOperand(onnx_node.name, mul_op)
                ...
                pow_op = operators.PowOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                                   base_op,
                                   exponent=expn_const,
                                   loc=self.get_loc("{}_{}".format(onnx_node.name,
                                                                   onnx_node.op_type)),
            ...
            pow_op = operators.Pow2Op(self.valueInfoToTensorType(onnx_node.outputs[0]),
                                base_const,
                                expn_op,
                                loc=self.get_loc("{}_{}".format(onnx_node.name, onnx_node.op_type)),
                                ip=self.mlir.insert_point).output
    ...
    def convert_where_op(self, onnx_node):
        ...
        #             )  # do not support broadcastable case recently
        if num_const == 0:
            new_op = operators.WhereOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                                 cond_opd,
                                 tbrn_opd,
                                 fbrn_opd,
                                 x_is_const=False,
    ...
    def convert_not_op(self, onnx_node):
        ...
        not_op = operators.CompareConstOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                                    self.getOp(opd),
                                    mode=StringAttr.get(onnx_node.op_type),
    ...
    def convert_cmp_op(self, onnx_node):
        ...
        if self.isScalar(lhs):
            ...
            cmp_op = operators.CompareConstOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                                        rhs_opd,
                                        mode=StringAttr.get(onnx_node.op_type),
                                        const_val=self.getScalar(lhs),
                                        inversed=True,
            ...
            rhs_opd = self.getOp(rhs)
            lhs_opd = self.getOp(lhs)
            cmp_op = operators.CompareOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                                   lhs_opd,
                                   rhs_opd,
                                   mode=StringAttr.get(onnx_node.op_type),
                                   loc=self.get_loc("{}_{}".format(onnx_node.name,
    ...
    def convert_hsigmoid_op(self, onnx_node):
        ...
        new_op = operators.HardSigmoidOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                                   operand,
                                   alpha=alpha,
                                   beta=beta,
                                   loc=self.get_loc("{}_{}".format(onnx_node.name,
    ...
    def convert_hswish_op(self, onnx_node):
        ...
        assert (len(onnx_node.inputs) == 1)
        operand = self.getOp(onnx_node.inputs[0])
        new_op = operators.HardSwishOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                                 operand,
                                 loc=self.get_loc("{}_{}".format(onnx_node.name,
                                                                 onnx_node.op_type)),
                                 ip=self.mlir.insert_point).output
    ...
    def convert_gelu_op(self, onnx_node):
        ...
        assert (len(onnx_node.inputs) == 1)
        operand = self.getOp(onnx_node.inputs[0])
        new_op = operators.GELUOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                            operand,
                            loc=self.get_loc("{}_{}".format(onnx_node.name, onnx_node.op_type)),
                            ip=self.mlir.insert_point).output
        self.addOperand(onnx_node.name, new_op)
    ...
    def convert_layer_norm_op(self, onnx_node):
        ...
        if len(onnx_node.inputs) > 2:
            if not self.isScalar_(onnx_node.inputs[2], 0):
                bias_opd = self.getWeightOp(onnx_node.inputs[2])
        out_op = operators.LayerNormOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                                 input_opd,
                                 scale_opd,
                                 bias_opd,
                                 normalized_shape=[],
    ...
    def convert_pixel_norm_op(self, onnx_node):
        ...
        if len(onnx_node.inputs) > 2:
            if not self.isScalar_(onnx_node.inputs[2], 0):
                bias_opd = self.getWeightOp(onnx_node.inputs[2])
        new_op = operators.PixelNormOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                                 input_opd,
                                 scale_opd,
                                 bias_opd,
                                 eps=eps,
    ...
    def convert_instance_norm_op(self, onnx_node):
        ...
        if len(onnx_node.inputs) > 2:
            if not self.isScalar_(onnx_node.inputs[2], 0):
                bias_opd = self.getWeightOp(onnx_node.inputs[2])
        new_op = operators.InstanceNormOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                                    input_opd,
                                    scale_opd,
                                    bias_opd,
                                    eps=eps,
    ...
    def convert_group_norm_op(self, onnx_node):
        ...
        if len(onnx_node.inputs) > 2:
            if not self.isScalar_(onnx_node.inputs[2], 0):
                bias_opd = self.getWeightOp(onnx_node.inputs[2])
        new_op = operators.GroupNormOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                                 input_opd,
                                 scale_opd,
                                 bias_opd,
    ...
    def convert_scatter_elements_op(self, onnx_node):
        ...
        new_op = operators.ScatterElementsOp(
            self.valueInfoToTensorType(onnx_node.outputs[0]),
            input,
            indices,
            updates,
            axis=axis,
    ...
    def convert_scatternd_op(self, onnx_node):
        ...
        reduction = onnx_node.attrs.get("reduction", None)
        assert not reduction
        scatternd_op = operators.ScatterNDOp(
            self.valueInfoToTensorType(onnx_node.outputs[0]),
            input_data,
            indices,
            updates,
    ...
    def convert_nonzero_op(self, onnx_node):
        ...
        assert (len(onnx_node.inputs) == 1)
        input_data = self.getOp(onnx_node.inputs[0])
        new_op = operators.NonZeroOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                               input_data,
    ...
    def convert_grid_sampler_op(self, onnx_node):
        ...
        new_op = operators.GridSamplerOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                                   input_data,
                                   grid_data,
                                   mode=mode,
    ...
    def convert_cumsum_op(self, onnx_node):
        ...
        operands = list()
        operands.append(self.getOp(onnx_node.inputs[0]))
        operands.append(self.getWeightOp(onnx_node.inputs[1]))
        new_op = operators.CumSumOp(self.valueInfoToTensorType(onnx_node.outputs[0]), *operands,
                            axis=axis,
                            loc=self.get_loc("{}_{}".format(onnx_node.name, onnx_node.op_type)),
                            ip=self.mlir.insert_point).output
        self.addOperand(onnx_node.name, new_op)
    ...
    def convert_round_op(self, onnx_node):
        ...
        operand = self.getOp(onnx_node.inputs[0])
        new_op = operators.RoundOp(self.valueInfoToTensorType(onnx_node.outputs[0]),
                            operand,
                            loc=self.get_loc("{}_{}".format(onnx_node.name, onnx_node.op_type)),
                            ip=self.mlir.insert_point).output
        self.addOperand(onnx_node.name, new_op)
