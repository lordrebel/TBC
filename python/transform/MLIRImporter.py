...
class CompilePhase:
    IMPORTED = 'IMPORTED'
    OPERATOR_OPTED = 'OPERATOR_OPTED'
    KERNEL = 'KERNEL'
    KERNEL_OPTED = 'KERNEL_OPTED'
    HAL = 'HAL'
    HAL_OPTED = 'HAL_OPTED'
    HAL_ADDRESSED = 'HAL_ADDRESSED'
    CODEGEN = 'CODEGEN'
...
class MLIRImporter(object):
    ...
    def get_tensor_type(self, output_shapes, type=None):
        ...
        if isinstance(type, str):
            if type in self.mlir_type:
                type = self.mlir_type[type]
            else:
                raise RuntimeError("unknown type:{}".format(type))
        if isinstance(type, list):
            type=[self.mlir_type[t] if isinstance(t, str) else t for t in type]
    ...
    def create_input_op(self, loc, index, kargs: dict = {}):
        ...
        init_args["output"] = self.input_types[index]
        input_op = operators.InputOp(**init_args)
    ...
    def create_weight_op(self, name, output_shape, data_type="F32"):
        ...
        op = Operation.create("operators.Weight",
                              results=[tensor_type],
    ...
    def create_yield_op(self, Operands):
        yield_op = Operation.create("operators.Yield", operands=Operands, results=[])
    ...
    def create_if_op(self, operands, output_shape, **kargs):
        ...
        op = Operation.create("operators.If",
    ...
    def create_loop_op(self, operands, output_shape, **kargs):
        region = IntegerAttr.get(self.mlir_type['INT64'], kargs["region"]).value
        op = Operation.create("operators.Loop",
                              results=self.get_tensor_type(output_shape),
                              operands=operands,
                              loc=Location.fused([Location.name(x) for x in kargs['name']]),
                              attributes=dict(),
    ...
    def create_subgraph_input_op(self, name, type, val, **kargs):
        param = {}
        op = Operation.create("operators.Input",
                              results=[type],
                              operands=[val],
                              loc=Location.fused([Location.name(name)]),
                              attributes=param)
    ...
    def declare_func(self, input_types: list = [], output_types: list = []):
        ...
        """.format(name=self.model_name,
                   ...
                   compile_phase=self.compilephase,
                   target=self.target,
                   args=args_txt,
                   output=output_txt,
