...
class TorchInterpreter():
    ...
    def run_aten(self, node: TorchNode):
        ...
        ParamMap = {
            ...
            "aten::full": ['dtype', 'layout', 'device', 'pin_memory'],
        ...
        if node.op_type == "aten::div":
            ...
            if node.op_type.split('::')[1] == 'full' or node.op_type.split('::')[1] == 'arange':
                output = func(*input_list)
            else:
                try:
                    ...
                    if "not implemented for \'Half\'" in err.args[0] and torch.cuda.is_available(
                    ) == False:
                        for i in range(len(input_list)):
                            if torch.is_tensor(
                                    input_list[i]) and input_list[i].dtype == torch.float16:
                                input_list[i] = input_list[i].float()
                                outdtype_fp32_to_fp16 = True
                        output = func(*input_list, **param_dict)
        else:
            try:
                output = func(*input_list)
                ...
                if "not implemented for \'Half\'" in err.args[0] and torch.cuda.is_available(
                    ...
                    for i in range(len(input_list)):
                        if torch.is_tensor(input_list[i]) and input_list[i].dtype == torch.float16:
                            input_list[i] = input_list[i].float()
                            outdtype_fp32_to_fp16 = True
                    output = func(*input_list)
                else:
                    raise
        if outdtype_fp32_to_fp16:
