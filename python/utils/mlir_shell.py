...
def mlir_opt_for_operator(mlirfile, opt_mlirfile):
    ...
    cmd.extend([
        "--canonicalize", " --assign-compile-phase=\"compile_phase=OPERATOR_OPTED\" ", "-o",
        opt_mlirfile
    ])
    _os_system(cmd)


def mlir_lowering_to_kernel(ops_mlir: str, kernels_mlir: str, chip: str):

    cmd = ["tbc-opt", ops_mlir, "--convert-operators-to-kernels"]
    cmd.extend([
        f"--assign-target=\"target={chip}\" ", "--target-depend-pass", " --op-reorder", " -o",
        kernels_mlir
    ])
    _os_system(cmd)


def mlir_opt_for_kernel(kls_mlir: str, opt_kls_mlir: str):
    pass


def mlir_lowering_to_hal(kls_mlir: str, hals_mlir: str):
    cmd = [
        "tbc-opt", kls_mlir, "--convert-kernels-to-hals", "--assign-tensorInfo", "--pack-weights",
        "--fuse-packed-weight-group-to-one", "--layer-group"
    ]
    cmd.extend([" -o ", hals_mlir])
    _os_system(cmd)


def mlir_opt_for_hal(hals_mlir: str, opt_hal_mlir: str):
    pass
...
# OperatorTOTOSA
def operator_to_tosa(top_mlir: str, tosa_mlir: str, includeWeight: bool = False):
